...
## Phase 1: Skeleton
1. Initialize Next.js with TypeScript, Tailwind, ESLint.
2. Setup `next-intl` with `/[locale]` routing.
3. Setup Prisma with Supabase & Clerk.

## Phase 2: Core Store
Create `useDesignStore` with `immer`. Define `Part` type containing `position`, `rotation`, and `specs`.

## Phase 3: 3D Editor
1. Create `CanvasArea` with R3F.
2. Implement `useSnapping` hook using Raycaster for socket detection.
3. Implement `Dnd-Kit` drag from Sidebar to Canvas.

## Phase 4: Logic & Export
1. Implement `useValidation` hook (Client-side physics check).
2. Implement client-side PDF/Word generation.
